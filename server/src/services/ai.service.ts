  ...
  const scored = candidates.map((c) => {
    ...
    if (keywords.length > 0) {
      ...
      for (const k of keywords.slice(0, 8)) {
        if (k && lower.includes(k.toLowerCase())) strengths.push(`Mentions "${k}"`);
      }
    }
    if (strengths.length === 0) gaps.push('Few explicit overlaps with job keywords detected in resume text.');
    ...
    const recommendation =
      score >= 80 ? 'Strong shortlist candidate.' : score >= 60 ? 'Potential fit; review key gaps.' : 'Low match; unlikely to meet requirements.';
