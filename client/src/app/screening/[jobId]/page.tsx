...
import { 
  ...
  ShieldCheck, ShieldAlert, Sparkles, FileWarning
} from 'lucide-react';
...
export default function ScreeningPage() {
  ...
  const handleDeleteCandidate = async (resultId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm('Are you sure you want to remove this candidate from this job?')) return;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div className="space-y-4">
          ...
          <h1 className="text-5xl font-black text-white tracking-tighter">
            {job?.title} <span className="text-slate-400">/ Results</span>
          </h1>
          <p className="text-slate-400 max-w-xl text-lg">
            Our AI has processed <span className="text-white font-bold">{results.length} candidates</span> against your requirements. 
        ...
        <div className="flex items-center gap-4">
          ...
          <label className="flex items-center gap-2 bg-slate-800/60 hover:bg-slate-700/80 text-slate-200 px-6 py-3 rounded-2xl cursor-pointer border border-slate-600/50 shadow-xl transition-all group">
            <Upload size={18} className="text-blue-400 group-hover:-translate-y-1 transition-transform" />
            <span className="font-bold">{uploading ? 'Processing...' : 'Add Candidates'}</span>
            <input type="file" className="hidden" accept=".pdf,.csv,.xlsx,.json,.txt" onChange={handleFileUpload} disabled={uploading} />
          </label>
        </div>
      </div>
            ...
            <AnimatePresence>
              {sortedResults.map((result, idx) => (
                <motion.div 
                  ...
                  className={`group relative p-6 rounded-[2rem] border-2 transition-all cursor-pointer overflow-hidden ${
                    selectedCandidate?._id === result._id 
                      ? 'bg-slate-700/80 border-blue-500 shadow-2xl shadow-blue-500/20' 
                      : 'bg-slate-800/50 border-slate-700/50 hover:border-slate-600'
                  }`}
                >
                  {result.rank === 1 && (
                    <div className="absolute top-0 right-0 bg-gradient-to-l from-emerald-500/20 to-transparent pl-20 pr-6 py-2">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                    <div className="flex items-center gap-6">
                      <div className="relative">
                        <div className="w-16 h-16 bg-slate-700 rounded-2xl flex items-center justify-center border border-slate-600 text-white text-2xl font-black">
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          {result.recommendation === 'Strong Hire' && (
                            <span className="bg-emerald-500/10 text-emerald-500 text-[9px] font-black px-2 py-0.5 rounded border border-emerald-500/20 uppercase tracking-widest">
                              Strong Hire
                            </span>
                          )}
                        </div>
                        <p className="text-slate-400 text-sm font-medium">{result.applicantId.headline || 'Product Manager'}</p>
                    ...
                    <div className="flex items-center gap-8">
                      <div className="flex flex-col items-end">
                        <div className="text-[10px] text-slate-400 uppercase tracking-widest font-black mb-1">AI Score</div>
                        <div className={`text-4xl font-black italic tracking-tighter ${
                          result.aiScore >= 80 ? 'text-emerald-500' : 
                          result.aiScore >= 60 ? 'text-blue-400' : 'text-slate-400'
                        }`}>
                          {result.aiScore}%
                        </div>
                      </div>
            ...
            <AnimatePresence mode="wait">
              {selectedCandidate ? (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -20 }}
                  className="bg-slate-800/60 border border-slate-700/60 rounded-[2.5rem] p-8 overflow-hidden relative shadow-2xl backdrop-blur-sm"
                >
                  ...
                  <div className="flex items-center justify-between mb-10">
                    ...
                    <div className="text-3xl font-black text-white italic opacity-20">#{selectedCandidate.rank}</div>
                  </div>

                  <div className="space-y-10 relative z-10">
                    <section>
                       <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Pipeline Status</div>
                            ...
                            <button
                              ...
                              className={`py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border-2 ${
                                selectedCandidate.decision === dec
                                  ...
                                  : 'bg-slate-700/50 text-slate-400 border-slate-600 hover:border-slate-500 hover:text-white'
                              } disabled:opacity-50`}
                            >
                    ...
                    <section className="bg-slate-100/30 p-6 rounded-3xl border border-slate-200/50 relative overflow-hidden">
                       ...
                       <div className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                         <TrendingUp size={14} />
                       ...
                       <div className="flex items-center gap-2 text-xs text-blue-400/60 font-bold italic">
                          <Info size={14} />
                    ...
                    <section>
                      <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Explainable Breakdown</div>
                      <div className="space-y-4">
                        {[
                          ...
                          return (
                            <div key={factor.key} className="space-y-2 group">
                              <div className="flex justify-between items-center text-[10px] font-bold">
                                 <span className="text-slate-400 uppercase tracking-widest">{factor.label}</span>
                                 <span className="text-white bg-slate-700/60 px-2 py-0.5 rounded">{score}%</span>
                              </div>
                ...
                <div className="bg-slate-800/40 border-2 border-slate-700 border-dashed rounded-[3rem] p-16 text-center text-slate-400 transition-all hover:border-slate-600">
                  <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 4, repeat: Infinity }}>
                    <User className="mx-auto mb-6 opacity-20" size={80} />
                  </motion.div>
                  <h3 className="text-lg font-black text-slate-400 mb-2 uppercase tracking-tighter">Select Candidate</h3>
                  <p className="text-sm">Explore deep-dive AI evaluation matrix and potential growth trajectory.</p>
                </div>
              )}
            </AnimatePresence>
          </div>
